...
Initially, I began by treating the effects of co-administered IPV and tOPV as independent (see function below). Under this model when estimating type 2 immunity, if a child did not receive tOPV before the IPV/tOPV campaign, then they are classified as 'naive'. Many younger children will have been exposed to a few tOPV campaigns in the latter half of 2014 (there was a 1yr+ absence of tOPV in northern Nigeria prior to August 2014), but the IPV campaign will have been first. To be better estimate immunity, we will need to flag NP-AFP records with multiple tOPV exposures where the IPV campaign was the first tOPV exposure.[^7] 

However, if we assume that the immunological effect of tOPV is on a much shorter time scale than IPV, then all children who receive IPV in Northern Nigeria will respond like OPV experienced children. This needs to be discussed further, but there is currently no trial data to support either conclusion.
...
ipv.opvback.t2 = .950*.75
calcImmunity(tOPV=1, bOPV=8, mOPV1=0, mOPV3=0, IPV=1, type=2, doses=4, ipv.random=F) # Population result approximated by scaling the IPV efficacy by coverage
ipv.opvback.t2 = .950 # Resent IPV dose efficacy
```  
...
One method would be to take individual NP-AFP coverage for individuals (total doses / number exposed) before the campaign and use that to project the number of doses at the end of the time period. Everyone in that time period would then be assumed to have been exposed to all of the campaigns in the time period (including the IPV campaign). Based on the current timing of the vaccinations (in June and Mid-August), most 6-59-month old children in the second half of the year will have been exposed.
...
[^7]: I am totally igoring RI here. We usually think this assumtion is safe in 